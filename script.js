document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getErrorMessage(field) {
      ...
      if (name.includes('name') && value !== '' && value.length < 2) {
        return field.dataset.errorName || 'Please enter a valid name.';
      }
    ...
    contactForm.addEventListener('blur', function (e) {
      ...
      const type = field.type ? field.type.toLowerCase() : '';
      const isValidatable = ['input', 'textarea', 'select'].includes(field.tagName.toLowerCase()) &&
        type !== 'submit' && type !== 'button' && type !== 'reset' && type !== 'hidden';
  ...
  style.textContent = [
    '.' + ANIMATE_CLASS + '{opacity:0;transform:translateY(24px);transition:opacity 0.6s ease,transform 0.6s ease;}',
    '.' + ANIMATE_CLASS + '.' + VISIBLE_CLASS + '{opacity:1;transform:translateY(0);}'
  ].join('');
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "קמפיין פריצה משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'd718a562-4488-4073-84ba-4742d559cd41',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'd718a562-4488-4073-84ba-4742d559cd41';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'd718a562-4488-4073-84ba-4742d559cd41';
